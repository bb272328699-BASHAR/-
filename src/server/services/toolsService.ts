...
export const ToolsService = {
  async getAll(params: {
    ...
    const cacheKey = `tools:list:${JSON.stringify(params)}`;
    return serverCache.wrap(cacheKey, async () => {
    ...
    }, 10 * 60 * 1000, ['tools', 'tools:list']);
  },

  async getBySlug(slug: string) {
    const decodedSlug = decodeURIComponent(slug || '').trim().toLowerCase();
    const cacheKey = `tools:slug:${decodedSlug}`;
    ...
    }, 15 * 60 * 1000, ['tools', `tool:${decodedSlug}`, 'tools:detail']);
  },

  async getFeaturedCollections() {
    return serverCache.wrap('collections:home', async () => {
    ...
    }, 10 * 60 * 1000, ['tools', 'collections', 'home']);
  }
};
