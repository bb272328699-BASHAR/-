...
import { 
  subscribeToFirestoreToolClicks, 
  fetchFirestoreToolClicks, 
  FirestoreToolClick,
  subscribeToLivePresence,
  FirestoreLivePresence
...
export const AdminAnalyticsDashboard: React.FC<AdminAnalyticsDashboardProps> = ({ token }) => {
  ...
  // Firestore Real-Time Click Tracker & Live Presence State
  const [firestoreClicks, setFirestoreClicks] = useState<FirestoreToolClick[]>([]);
  const [liveVisitors, setLiveVisitors] = useState<FirestoreLivePresence[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribeClicks = subscribeToFirestoreToolClicks((clicks) => {
      setFirestoreClicks(clicks);
      setClicksLoading(false);
    }, 150);

    // Subscribe to real-time live visitors presence
    const unsubscribePresence = subscribeToLivePresence((visitors) => {
      setLiveVisitors(visitors);
    });

    return () => {
      if (typeof unsubscribeClicks === 'function') unsubscribeClicks();
  ...
  return (
    <div className="space-y-8" dir="rtl">
      ...
      {/* Real-Time Live Presence Pulse Widget */}
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-indigo-500/30 rounded-3xl p-6 text-white shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-black text-white">الزوار النشطون المتصلون بالموقع الآن (Live Presence)</h3>
              ...
              <p className="text-xs text-slate-300 pt-0.5">
                تحديث لحظي فور وصول أي زائر دون الحاجة لتحديث الصفحة - نبض متصل بدقة 100%
              </p>
            </div>
          </div>

          <div className="flex items-center gap-3 bg-white/10 px-5 py-2.5 rounded-2xl border border-white/10">
            <span className="text-xs text-slate-300 font-bold">المتصلون الآن:</span>
            <span className="text-2xl font-black text-emerald-400 font-mono">
              {liveVisitors.length > 0 ? liveVisitors.length : (summary.realTimeActiveUsers || 0)}
            </span>
            <span className="text-xs text-slate-400">زائر</span>
          </div>
        </div>

        {/* Live Active Visitor Detail Cards */}
        {liveVisitors.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-2">
            {liveVisitors.slice(0, 6).map((visitor) => (
              <div key={visitor.id} className="p-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl space-y-1.5 transition-all text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-indigo-300 truncate font-mono" dir="ltr">
                    {visitor.pagePath}
                  </span>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400">
                  <span>📱 {visitor.device} ({visitor.os || 'Unknown'})</span>
          ...
          <div className="text-xs text-slate-400 bg-white/5 rounded-2xl p-3 text-center border border-white/5">
            لا توجد جلسات مفتوحة في هذه اللحظة بالضبط. بمجرد دخول أي زائر حقيقي سيظهر ملفه وصفحته هنا فوراً تلقائياً.
          </div>
        )}
      </div>
